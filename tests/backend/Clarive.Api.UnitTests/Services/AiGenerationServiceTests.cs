...
public class AiGenerationServiceTests
{
    ...
    public async Task GenerateAsync_PersistsSessionAndReturnsResult()
    {
        ...
        _orchestrator.GenerateAsync(Arg.Any<GenerationConfig>(), Arg.Any<CancellationToken>(),
    ...
    public async Task GenerateAsync_WithToolIds_BuildsConfigWithTools()
    {
        ...
        _toolRepo.GetByIdsAsync(TenantId, Arg.Any<List<Guid>>(), Arg.Any<CancellationToken>())
        ...
        await _orchestrator.Received(1).GenerateAsync(
            ...
            Arg.Any<CancellationToken>(), Arg.Any<Func<ProgressEvent, Task>?>());
    }
    ...
    public async Task GenerateSystemMessageAsync_AlreadyHasSystemMessage_ReturnsConflict()
    {
        _entryRepo.GetByIdAsync(TenantId, EntryId, Arg.Any<CancellationToken>())
            .Returns(new PromptEntry { Id = EntryId, IsTrashed = false });
        _entryRepo.GetWorkingVersionAsync(TenantId, EntryId, Arg.Any<CancellationToken>())
            .Returns(new PromptEntryVersion
            {
