...
public class EntryTagTests : EntryServiceTestBase
{
    ...
    public async Task AddEntryTagsAsync_EntryNotFound_ReturnsNotFound()
    {
        ...
        var result = await Sut.AddEntryTagsAsync(TenantId, Guid.NewGuid(), ["tag1"], CancellationToken.None);
    ...
    public async Task AddEntryTagsAsync_TagTooLong_ReturnsValidationError()
    {
        ...
        var result = await Sut.AddEntryTagsAsync(TenantId, entry.Id, [longTag], CancellationToken.None);
    ...
    public async Task AddEntryTagsAsync_InvalidPattern_ReturnsValidationError()
    {
        ...
        var result = await Sut.AddEntryTagsAsync(TenantId, entry.Id, ["invalid@tag!"], CancellationToken.None);
    ...
    public async Task AddEntryTagsAsync_ValidTags_NormalizesAndReturnsUpdatedList()
    {
        ...
        EntryRepo.GetByIdAsync(TenantId, entry.Id, Arg.Any<CancellationToken>())
            .Returns(entry);
        TagRepo.GetByEntryIdAsync(TenantId, entry.Id, Arg.Any<CancellationToken>())
            .Returns(new List<string> { "existing", "new-tag" });

        var result = await Sut.AddEntryTagsAsync(TenantId, entry.Id, ["  New-Tag  ", "NEW-TAG"], CancellationToken.None);

        result.IsError.Should().BeFalse();
        result.Value.Should().Contain("new-tag");

        // Verify normalized and deduplicated tags were passed to repo
        await TagRepo.Received(1).AddAsync(TenantId, entry.Id,
    ...
    public async Task RemoveEntryTagAsync_EntryNotFound_ReturnsNotFound()
    {
        ...
        var result = await Sut.RemoveEntryTagAsync(TenantId, Guid.NewGuid(), "tag1", CancellationToken.None);
    ...
    public async Task RemoveEntryTagAsync_ValidTag_NormalizesAndRemoves()
    {
        ...
        var result = await Sut.RemoveEntryTagAsync(TenantId, entry.Id, "  My-Tag  ", CancellationToken.None);

        result.IsError.Should().BeFalse();
        await TagRepo.Received(1).RemoveAsync(TenantId, entry.Id, "my-tag", Arg.Any<CancellationToken>());
    }
}
