...
public class EntryFavoriteTests : EntryServiceTestBase
{
    ...
    public async Task FavoriteEntryAsync_EntryNotFound_ReturnsNotFound()
    {
        ...
        var result = await Sut.FavoriteEntryAsync(TenantId, UserId, Guid.NewGuid(), CancellationToken.None);
    ...
    public async Task FavoriteEntryAsync_AlreadyFavorited_ReturnsSuccessWithoutDuplicate()
    {
        ...
        EntryRepo.GetByIdAsync(TenantId, entry.Id, Arg.Any<CancellationToken>())
            .Returns(entry);
        FavoriteRepo.ExistsAsync(TenantId, UserId, entry.Id, Arg.Any<CancellationToken>())
            .Returns(true);

        var result = await Sut.FavoriteEntryAsync(TenantId, UserId, entry.Id, CancellationToken.None);

        result.IsError.Should().BeFalse();
        await FavoriteRepo.DidNotReceive().AddAsync(Arg.Any<EntryFavorite>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task FavoriteEntryAsync_NotYetFavorited_AddsFavorite()
    {
        ...
        result.IsError.Should().BeFalse();
        await FavoriteRepo.Received(1).AddAsync(
            Arg.Is<EntryFavorite>(f => f.TenantId == TenantId && f.UserId == UserId && f.EntryId == entry.Id),
    ...
    {
        var result = await Sut.UnfavoriteEntryAsync(TenantId, UserId, Guid.NewGuid(), CancellationToken.None);
