...
public class AuthServiceTests
{
    ...
    public AuthServiceTests()
    {
        ...
        _sut = new AuthService(
            _userRepo, _tokenRepo, _refreshTokenRepo,
            _emailService, appSettings, _jwtService, _passwordHasher);
    }
    ...
    {
        _tokenRepo.GetVerificationByHashAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
    ...
    public async Task VerifyEmailAsync_AlreadyVerified_MarksTokenUsedAndReturnsMessage()
    {
        ...
        await _tokenRepo.Received(1).MarkVerificationUsedAsync(tokenId, Arg.Any<CancellationToken>());
    ...
    public async Task ResendVerificationAsync_RateLimited_Returns429()
    {
        ...
        _tokenRepo.CountRecentVerificationTokensAsync(UserId, Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>())
            .Returns(1);
    ...
    {
        var user = new User { Id = UserId, Email = "test@test.com", Name = "Test", EmailVerified = false };
        _userRepo.GetByIdAsync(TenantId, UserId, Arg.Any<CancellationToken>())
            .Returns(user);
        _tokenRepo.CountRecentVerificationTokensAsync(UserId, Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>())
            .Returns(0);
        ...
        await _emailService.Received(1).SendVerificationEmailAsync(
            "test@test.com", "Test", Arg.Is<string>(u => u.Contains("/verify-email?token=")), Arg.Any<CancellationToken>());
    }
    ...
    public async Task ForgotPasswordAsync_RateLimited_DoesNotSendEmail()
    {
        ...
        _userRepo.GetByEmailAsync("test@test.com", Arg.Any<CancellationToken>())
            .Returns(user);
        _tokenRepo.CountRecentResetTokensAsync(UserId, Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>())
            .Returns(3);

        await _sut.ForgotPasswordAsync("test@test.com", default);

        await _tokenRepo.DidNotReceive().CreateResetTokenAsync(
    ...
    public async Task ForgotPasswordAsync_Valid_CreatesTokenAndSendsEmail()
    {
        ...
        await _tokenRepo.Received(1).CreateResetTokenAsync(
            Arg.Any<PasswordResetToken>(), Arg.Any<CancellationToken>());
        await _emailService.Received(1).SendPasswordResetEmailAsync(
            "test@test.com", "Test", Arg.Is<string>(u => u.Contains("/reset-password?token=")), Arg.Any<CancellationToken>());
    }
    ...
    {
        _tokenRepo.GetResetByHashAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
