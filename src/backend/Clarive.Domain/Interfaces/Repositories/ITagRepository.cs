...
{
    Task<List<(string TagName, int EntryCount)>> GetAllWithCountsAsync(
        Guid tenantId,
    ...
    Task<List<string>> GetByEntryIdAsync(
        Guid tenantId,
        Guid entryId,
        CancellationToken ct = default
    );
    Task<Dictionary<Guid, List<string>>> GetByEntryIdsBatchAsync(
        Guid tenantId,
        List<Guid> entryIds,
        CancellationToken ct = default
    );
    ...
    Task<HashSet<Guid>> GetEntryIdsByTagsAsync(
        Guid tenantId,
        List<string> tags,
        bool matchAll,
        CancellationToken ct = default
