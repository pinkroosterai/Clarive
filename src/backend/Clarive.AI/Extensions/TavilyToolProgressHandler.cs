...
public sealed class TavilyToolProgressHandler
{
    ...
    public async Task OnToolCallStartingAsync(object sender, ToolCallStartingEventArgs e)
    {
        if (_reporter.OnProgress is not { } callback)
            return;
        if (!TavilyTools.Contains(e.FunctionName))
            return;
    ...
    private static ProgressEvent? FormatStartEvent(
        string callId,
        string toolName,
    ...
    private static ProgressEvent FormatSearch(string callId, AIFunctionArguments? args)
    {
        ...
        return ProgressEvent.ToolStart(
            callId,
            "\U0001f50d",
            "Searching\u2026",
            query is not null ? query.Truncate(80) : null
        );
    }
    ...
    private static ProgressEvent FormatExtract(string callId, AIFunctionArguments? args)
    {
        ...
        return ProgressEvent.ToolStart(
            ...
            "\U0001f4c4",
            "Reading content\u2026",
            url is not null ? url.Truncate(60) : null
        );
    }
