    ...
    PasswordHasher passwordHasher) : IProfileService
{
        ...
        Guid tenantId, Guid userId, UpdateProfileRequest request, CancellationToken ct)
    {
        ...
        if (request.NewPassword is not null && user.PasswordHash is null)
            return Error.Validation("VALIDATION_ERROR", "Password changes are not available for accounts using external sign-in.");

        if ((request.Email is not null || request.NewPassword is not null)
            && string.IsNullOrWhiteSpace(request.CurrentPassword))
            return Error.Validation("VALIDATION_ERROR", "Current password is required to change email or password.");
        ...
        if (request.Name is not null)
        {
            if (request.Name.Length > 255)
                return Error.Validation("VALIDATION_ERROR", "Name must be 255 characters or fewer.");
        ...
        if (request.ThemePreference is not null)
        {
            if (!ValidThemePreferences.Contains(request.ThemePreference))
                return Error.Validation("VALIDATION_ERROR", "Theme preference must be 'light', 'dark', or 'system'.");
