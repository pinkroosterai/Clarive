    ...
    PasswordHasher passwordHasher) : ISuperAdminService
{
    public async Task<PlatformStatsResponse> GetPlatformStatsAsync(CancellationToken ct)
    {
        ...
        var userStats = await db.Users.IgnoreQueryFilters()
        ...
        var membershipStats = await db.TenantMemberships.IgnoreQueryFilters()
        ...
        var invitationStats = await db.Invitations.IgnoreQueryFilters()
            .GroupBy(_ => 1)
            .Select(g => new
            {
                Total = g.Count(),
        ...
        var invitationAcceptRate = totalInvitations > 0
            ? (double)invitationStats!.Accepted / totalInvitations
            : 0;

        // Content — single query for entries
        var entryStats = await db.PromptEntries.IgnoreQueryFilters()
            .GroupBy(_ => 1)
            .Select(g => new
            {
                Total = g.Count(),
        ...
        var aiStats = await db.AiSessions.IgnoreQueryFilters()
            .GroupBy(_ => 1)
            .Select(g => new
            {
                Total = g.Count(),
                Last7d = g.Count(s => s.CreatedAt >= days7),
            })
            .FirstOrDefaultAsync(ct);
