    ...
    ILogger<InvitationService> logger) : IInvitationService
{
        ...
        string email, UserRole role, CancellationToken ct)
    {
        ...
        if (existingUser is not null)
            return await CreateForExistingUserAsync(tenantId, invitedById, inviterName, normalizedEmail, role, existingUser, ct);

        return await CreateForNewUserAsync(tenantId, invitedById, inviterName, normalizedEmail, role, ct);
    }
    ...
    public async Task<ResendInvitationResult?> ResendAsync(
        Guid tenantId, Guid invitationId, string inviterName, CancellationToken ct)
    {
        ...
        if (invitation.TargetUserId is not null)
        {
            ...
            var targetUser = await userRepo.GetByIdCrossTenantsAsync(invitation.TargetUserId.Value, ct);
            targetUserName = targetUser?.Name ?? "there";
            var loginUrl = $"{appSettings.Value.FrontendUrl}/login";

            FireAndForgetEmail(() => emailService.SendWorkspaceInviteEmailAsync(
                invitation.Email, targetUserName, workspaceName,
                invitation.Role.ToString().ToLower(), inviterName, loginUrl, CancellationToken.None));
        }
        else
        {
            ...
            FireAndForgetEmail(() => emailService.SendInvitationEmailAsync(
                invitation.Email, inviterName, workspaceName,
                invitation.Role.ToString().ToLower(), acceptUrl, CancellationToken.None));
        }

        return new ResendInvitationResult(invitation, invitation.TargetUserId is not null, rawToken, targetUserName);
    }

    public async Task<Invitation?> RevokeAsync(Guid tenantId, Guid invitationId, CancellationToken ct)
    {
        var invitation = await invitationRepo.GetByIdAsync(tenantId, invitationId, ct);
        if (invitation is null)
            return null;
    ...
    public async Task<List<PendingInvitationInfo>> GetPendingAsync(Guid userId, CancellationToken ct)
    {
        ...
        {
            if (inv.ExpiresAt <= DateTime.UtcNow) continue;
            ...
            results.Add(new PendingInvitationInfo(
                inv.Id,
                tenant?.Name ?? "Unknown",
        ...
        string normalizedEmail, UserRole role, User existingUser, CancellationToken ct)
    {
        ...
        FireAndForgetEmail(() => emailService.SendWorkspaceInviteEmailAsync(
            normalizedEmail, existingUser.Name, workspaceName,
            role.ToString().ToLower(), inviterName, loginUrl, CancellationToken.None));

        await auditLogger.SafeLogAsync(tenantId, invitedById, inviterName, AuditAction.UserInvited,
        ...
        string normalizedEmail, UserRole role, CancellationToken ct)
    {
        ...
        var invitation = await invitationRepo.CreateAsync(new Invitation
        {
            Id = Guid.NewGuid(),
            TenantId = tenantId,
            Email = normalizedEmail,
        ...
        FireAndForgetEmail(() => emailService.SendInvitationEmailAsync(
            normalizedEmail, inviterName, workspaceName,
            role.ToString().ToLower(), acceptUrl, CancellationToken.None));
