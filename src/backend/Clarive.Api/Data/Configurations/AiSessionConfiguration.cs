...
public class AiSessionConfiguration : IEntityTypeConfiguration<AiSession>
{
    public void Configure(EntityTypeBuilder<AiSession> builder)
    {
        ...
        builder.Property(s => s.Draft).HasColumnName("draft")
            .HasColumnType("jsonb")
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<CreateEntryRequest>(v, (JsonSerializerOptions?)null)!)
            .IsRequired();
        builder.Property(s => s.Questions).HasColumnName("questions")
            .HasColumnType("jsonb")
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<List<ClarificationQuestion>>(v, (JsonSerializerOptions?)null)!)
            .IsRequired();
        builder.Property(s => s.Enhancements).HasColumnName("enhancements")
            .HasColumnType("jsonb")
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<List<string>>(v, (JsonSerializerOptions?)null)!)
            .IsRequired();
        builder.Property(s => s.ScoreHistory).HasColumnName("score_history")
            .HasColumnType("jsonb")
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<List<IterationScore>>(v, (JsonSerializerOptions?)null)!)
            .IsRequired();
        builder.Property(s => s.Config).HasColumnName("config")
            .HasColumnType("jsonb")
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<GenerationConfig>(v, (JsonSerializerOptions?)null));
        builder.Property(s => s.AgentSessionId).HasColumnName("agent_session_id")
            .HasMaxLength(64);
