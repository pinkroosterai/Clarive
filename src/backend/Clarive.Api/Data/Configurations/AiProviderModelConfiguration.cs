...
public class AiProviderModelConfiguration : IEntityTypeConfiguration<AiProviderModel>
{
    public void Configure(EntityTypeBuilder<AiProviderModel> builder)
    {
        ...
        builder.Property(m => m.InputCostPerMillion).HasColumnName("input_cost_per_million").HasPrecision(18, 6);
        builder.Property(m => m.OutputCostPerMillion).HasColumnName("output_cost_per_million").HasPrecision(18, 6);
        builder.Property(m => m.HasManualCostOverride).HasColumnName("has_manual_cost_override").IsRequired();
