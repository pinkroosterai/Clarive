...
public interface IToolRepository
{
    ...
    Task<(List<ToolDescription> Tools, int Total)> GetByTenantPagedAsync(Guid tenantId, int page, int pageSize, CancellationToken ct = default);
    Task<ToolDescription?> GetByIdAsync(Guid tenantId, Guid toolId, CancellationToken ct = default);
    Task<List<ToolDescription>> GetByIdsAsync(Guid tenantId, IEnumerable<Guid> toolIds, CancellationToken ct = default);
