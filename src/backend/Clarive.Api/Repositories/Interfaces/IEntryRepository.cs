...
{
    Task<(List<PromptEntry> Items, int TotalCount)> GetByFolderAsync(Guid tenantId, Guid? folderId, bool includeAll, int page = 1, int pageSize = 50, CancellationToken ct = default, IQueryable<Guid>? filteredEntryIds = null, string? search = null, string? status = null, string? sortBy = null);
