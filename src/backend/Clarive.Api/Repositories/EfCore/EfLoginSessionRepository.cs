...
{
    public async Task<LoginSession> CreateAsync(LoginSession session, CancellationToken ct = default)
    {
    ...
    public async Task<List<LoginSession>> GetByUserAsync(Guid userId, int limit = 50, CancellationToken ct = default)
    {
        var now = DateTime.UtcNow;

        return await db.LoginSessions
            .AsNoTracking()
            .Join(
    ...
    public async Task<bool> RevokeAsync(Guid userId, Guid sessionId, CancellationToken ct = default)
    {
        ...
        var updated = await db.RefreshTokens
            .Where(rt => rt.Id == session.RefreshTokenId && rt.RevokedAt == null)
    ...
    public async Task<int> RevokeAllExceptAsync(Guid userId, Guid currentRefreshTokenId, CancellationToken ct = default)
    {
