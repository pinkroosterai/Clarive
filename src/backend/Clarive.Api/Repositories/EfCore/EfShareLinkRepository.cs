...
{
    public async Task<ShareLink?> GetByTokenHashAsync(string tokenHash, CancellationToken ct = default)
    {
    ...
    public async Task<ShareLink?> GetByEntryIdAsync(Guid tenantId, Guid entryId, CancellationToken ct = default)
    {
        return await db.ShareLinks
            .AsNoTracking()
            .FirstOrDefaultAsync(s => s.TenantId == tenantId && s.EntryId == entryId && s.IsActive, ct);
    }
    ...
    public async Task<bool> DeleteByEntryIdAsync(Guid tenantId, Guid entryId, CancellationToken ct = default)
    {
        var link = await db.ShareLinks.FirstOrDefaultAsync(
            s => s.TenantId == tenantId && s.EntryId == entryId, ct);
    ...
    public async Task IncrementAccessCountAsync(Guid shareLinkId, CancellationToken ct = default)
    {
        await db.ShareLinks
            ...
            .ExecuteUpdateAsync(s => s.SetProperty(
                p => p.AccessCount, p => p.AccessCount + 1), ct);
    }
}
