...
function emptyResponse(status = 204): Response {
  return {
    ...
    type: "basic" as ResponseType,
    url: "",
    clone: () => emptyResponse(status) as Response,
    body: null,
    bodyUsed: false,
    arrayBuffer: () => Promise.resolve(new ArrayBuffer(0)),
    formData: () => Promise.resolve(new FormData()),
...
function errorResponse(
  ...
  return {
    ...
    type: "basic" as ResponseType,
    url: "",
    clone: () => errorResponse(status, code, message) as Response,
    body: null,
    bodyUsed: false,
    arrayBuffer: () => Promise.resolve(new ArrayBuffer(0)),
    formData: () => Promise.resolve(new FormData()),
...
describe("apiClient", () => {
  ...
  describe("api.get", () => {
    it("sends GET request with auth header", async () => {
      ...
      const result = await api.get<{ id: number; name: string }>(
        "/api/entries",
      );
  ...
  describe("error handling", () => {
    it("throws ApiError with parsed error body", async () => {
      mockFetch.mockResolvedValue(
        errorResponse(400, "VALIDATION_ERROR", "Title is required"),
      );
  ...
  describe("401 token refresh", () => {
    ...
    it("does NOT refresh on register endpoint", async () => {
      ...
      mockFetch.mockResolvedValue(
        errorResponse(401, "CONFLICT", "Email taken"),
      );
    ...
    it("does NOT refresh on google auth endpoint", async () => {
      ...
      mockFetch.mockResolvedValue(
        errorResponse(401, "INVALID_TOKEN", "Bad google token"),
      );
    ...
    it("deduplicates concurrent refresh calls", async () => {
      ...
      const refreshCalls = mockFetch.mock.calls.filter(
        ([url]: [string]) => url === `${BASE_URL}/api/auth/refresh`,
      );
  ...
  describe("api.download", () => {
    ...
    it("throws ApiError on failure", async () => {
      mockFetch.mockResolvedValue(
        errorResponse(403, "FORBIDDEN", "Not allowed"),
      );
