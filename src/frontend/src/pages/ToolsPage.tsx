...
export default function ToolsPanel() {
  ...
  const { data: tools, isLoading } = useQuery({ queryKey: ["tools"], queryFn: toolService.getToolsList });
  ...
  const formValid = name.trim() !== "" && toolName.trim() !== "" && TOOL_NAME_RE.test(toolName) && description.trim() !== "";
  ...
  const handleCreate = () => {
    if (!formValid) return;
    createMutation.mutate({ name: name.trim(), toolName: toolName.trim(), description: description.trim() });
  };

  const handleUpdate = useCallback(async (id: string, data: Partial<Omit<ToolDescription, "id">>) => {
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={addOpen} onOpenChange={(open) => { if (!open) resetAndClose(); else setAddOpen(true); }}>
          <DialogTrigger asChild>
            <Button><Plus className="size-4 mr-1" /> Add Tool</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="add-name" placeholder="e.g. Image Generator" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="add-toolName">Tool Name</Label>
                <Input id="add-toolName" placeholder="e.g. generate_image" value={toolName} onChange={(e) => setToolName(e.target.value)} className="font-mono" />
                {!toolNameValid && (
              ...
              <div className="space-y-2">
                ...
                <Textarea id="add-desc" placeholder="What does this tool do?" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} />
              </div>
            </div>
            <DialogFooter>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-xl bg-elevated skeleton-shimmer h-36 border border-border-subtle" />
          ))}
        </div>
