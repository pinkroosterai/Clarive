...
import { Monitor, Smartphone, Globe, Trash2, ShieldAlert, Loader2 } from 'lucide-react';
...
function getDeviceIcon(os: string) {
  ...
  if (mobile.includes(os)) return <Smartphone className="h-5 w-5 text-foreground-muted" />;
...
export default function SessionManager() {
  ...
  const revokeAllMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['sessions'] });
      toast.success(`${data.revoked} session${data.revoked !== 1 ? 's' : ''} revoked`);
    },
    onError: (err: unknown) => handleApiError(err, { fallback: 'Failed to revoke sessions' }),
  });
  ...
  if (isError) {
    return (
      <div className="space-y-4 max-w-2xl">
        <h2 className="text-lg font-semibold">Active Sessions</h2>
        <p className="text-sm text-foreground-muted">Failed to load sessions.</p>
      </div>
    );
  }
