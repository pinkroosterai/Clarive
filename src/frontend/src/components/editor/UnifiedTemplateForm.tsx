  ...
  while ((match = tagRegex.exec(content)) !== null) {
    ...
    if (hasValue) {
      ...
      elements.push(
        <span
          key={key++}
          className="rounded bg-warning-bg px-1 text-warning-text"
        >
          {match[0]}
        </span>,
      );
...
function RenderedPromptPreview({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="rounded-md bg-elevated p-3 text-sm whitespace-pre-wrap">
        {elements}
      </div>
    </div>
  );
...
export function UnifiedTemplateForm({ prompts, isReadOnly }: UnifiedTemplateFormProps) {
  ...
  const getValue = (field: TemplateField) =>
              ...
              {fields.map((field) => {
                ...
                return (
                  <div key={field.name} className="space-y-1.5">
                    ...
                    {error && (
                      <p className="text-[0.8rem] font-medium text-error-text">{error}</p>
                    )}
                  </div>
                );
              })}
