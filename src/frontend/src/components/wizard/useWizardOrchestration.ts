...
export function useWizardOrchestration(
  ...
  const runBootstrap = useCallback(() => {
    ...
    wizardService.enhanceEntry(existingEntryId, setCurrentStage).then((result) => {
      applyResult(result);
  ...
  const handleDescribe = useCallback(
    async (description: string, options: GenerateOptions) => {
      ...
      try {
        const result = await wizardService.preGenClarify(description, {
    ...
    async (
      ...
      try {
        const enhancementIndices = selectedEnhancementNames
          ...
          .filter((i) => i >= 0);
        const result = await wizardService.generatePrompt(pendingDescription, {
  ...
  const stepLabels = mode === "new"
    ? ["Describe", "Clarify", "Review", "Save"]
    : ["Review", "Save"];
  const prevStepRef = useRef(displayStep);
  const direction = displayStep >= prevStepRef.current ? "forward" : "backward";
  useEffect(() => { prevStepRef.current = displayStep; }, [displayStep]);

  const stepHint = (() => {
    switch (step) {
      case 1: return "Describe what you want to generate";
      case 2: return "Answer clarification questions or skip";
      case 3: return "Review the generated draft and optionally refine it";
      case 4: return "Save your prompt entry";
      default: return "";
    }
  })();
