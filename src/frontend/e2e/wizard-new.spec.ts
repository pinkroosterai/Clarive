...
test.describe("AI Wizard — New Mode", () => {
  ...
  test("step 1: enter description and proceed to step 2 (Clarify)", async ({
    page,
  }) => {
    await page.locator("#wizard-desc").fill(
      "A prompt that helps write professional emails given a topic and tone"
    );
    ...
    await expect(
      page.getByText("Help us refine your prompt")
  ...
  test("step 2: answer questions and proceed to step 3 (Review)", async ({
    page,
  }) => {
    // Navigate to step 2
    ...
    await expect(
      page.locator("input[placeholder='Entry title']")
    ).toHaveValue(MOCK_GENERATE_RESPONSE.draft.title);
  ...
  test("step 2: skip questions and proceed directly to review", async ({
    page,
  }) => {
    // Navigate to step 2
    await page.locator("#wizard-desc").fill("Test prompt description");
  ...
  test("step 3: review shows generated draft with quality scores", async ({
    page,
  }) => {
    // Navigate to step 3
    await page.locator("#wizard-desc").fill("Test prompt description");
    ...
    const dimensions = ["Clarity", "Specificity", "Structure", "Completeness", "Autonomy", "Faithfulness"];
  ...
  test("step 3: accept draft and proceed to step 4 (Save)", async ({
    page,
  }) => {
    // Navigate to step 3
    await page.locator("#wizard-desc").fill("Test prompt description");
    ...
    await expect(
      page.getByText("Save as new draft entry?")
    ).toBeVisible({ timeout: 5_000 });

    // Draft title should be shown
    await expect(
      page.getByText(MOCK_GENERATE_RESPONSE.draft.title)
    ).toBeVisible();
  ...
  test("step 4: save creates entry and navigates to editor", async ({
    page,
  }) => {
    ...
    await expect(page).toHaveURL(
      new RegExp(`/entry/${MOCK_CREATED_ENTRY.id}`)
    );
  });
  ...
  test("close button with draft shows discard confirmation", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("You will lose the generated draft")
    ).toBeVisible();
