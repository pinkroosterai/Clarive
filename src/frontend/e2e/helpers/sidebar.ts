 ...
 */
export async function openFolderMenu(
  page: Page,
  folderName: string
): Promise<void> {
  ...
  const row = nameSpan.locator("xpath=ancestor::div[contains(@class, 'flex') and contains(@class, 'items-center') and contains(@class, 'w-full')]").first();
  await row.hover();
