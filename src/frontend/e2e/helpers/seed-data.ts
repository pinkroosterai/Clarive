...
export const VISIBLE_ENTRIES = Object.values(ENTRIES).filter((e) => !('trashed' in e && e.trashed));

/** Draft entries visible in the library */
export const DRAFT_ENTRIES = VISIBLE_ENTRIES.filter((e) => e.state === 'draft');

/** Published entries visible in the library */
export const PUBLISHED_ENTRIES = VISIBLE_ENTRIES.filter((e) => e.state === 'published');
