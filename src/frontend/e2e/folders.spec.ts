 ...
 */
async function fillInlineInput(page: import("@playwright/test").Page, sidebar: ReturnType<typeof getSidebar>, name: string) {
  const input = sidebar.locator("input").last();
  await input.waitFor({ state: "visible", timeout: 5_000 });
  // Wait for any Radix dropdown menu to fully close (prevents focus theft)
  await page.locator("[role='menu']").waitFor({ state: "hidden" }).catch(() => {});
...
test.describe("Folder CRUD", () => {
  ...
  test("delete the child subfolder (empty folder)", async ({ page }) => {
    ...
    await sidebar.getByText(childName, { exact: false }).waitFor({ state: "visible", timeout: 5_000 });
  ...
  test("cannot delete non-empty folder", async ({ page }) => {
    ...
    await expect(
      getSidebar(page).getByText(FOLDERS.contentWriting.name)
    ).toBeVisible();
  });
});
