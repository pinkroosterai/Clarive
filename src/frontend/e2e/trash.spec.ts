...
test.describe('Trash — Delete, Restore & Permanent Delete', () => {
  ...
  test('create an entry and move it to trash from library', async ({ page }) => {
    // Create entry
  ...
  test('trash page shows empty state when no entries are trashed', async ({ page }) => {
    ...
    const hasItems = await content
      .getByText('Deprecated: Basic Summarizer')
      .isVisible()
      .catch(() => false);
    if (!hasItems) {
