...
test.describe("Viewer Restrictions", () => {
  ...
  test("viewer cannot edit entries — action buttons hidden", async ({
    page,
  }) => {
    await page.goto(`/entry/${ENTRIES.emailToneAdjuster.id}`);
    await page.waitForLoadState("networkidle");
  ...
  test("viewer cannot manage API keys or delete account", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Only the account admin can manage API keys/i)
    ).toBeVisible();
...
test.describe("Editor Permissions", () => {
  ...
  test("editor can create and edit but cannot manage settings", async ({
    page,
  }) => {
