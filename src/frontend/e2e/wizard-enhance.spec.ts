...
const EXISTING_ENTRY = {
  ...
  systemMessage: null,
  prompts: [
    { id: "p-existing-1", content: "Write something about {{topic}}.", order: 0 },
  ],
};
...
test.describe("AI Wizard — Enhance Mode", () => {
  ...
  test("enhance page loads, analyzes entry, and shows review step", async ({
    page,
  }) => {
  ...
  test("accept enhanced draft and save applies changes", async ({ page }) => {
    ...
    await expect(
      page.getByText("Apply enhanced version to current entry?")
    ...
    await expect(page).toHaveURL(
      new RegExp(`/entry/${EXISTING_ENTRY.id}`)
    );
  });
