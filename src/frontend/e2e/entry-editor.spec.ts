 ...
 */
async function createTestEntry(page: import("@playwright/test").Page, title: string): Promise<string> {
...
test.describe("Entry Editor — Publishing & Versioning", () => {
  ...
  test("publish draft → version badge changes to Published", async ({
    page,
  }) => {
    await page.goto(entryUrl);
    await page.waitForLoadState("networkidle");
  ...
  test("editing published entry creates a new draft version", async ({
    page,
  }) => {
    await page.goto(entryUrl);
    await page.waitForLoadState("networkidle");
  ...
  test("view historical version shows read-only banner", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Viewing v1.*read-only/)
    ).toBeVisible();
