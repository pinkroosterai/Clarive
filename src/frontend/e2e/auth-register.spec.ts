...
test.describe("Authentication — Registration", () => {
  ...
  test("register with unique email creates account and redirects to /library", async ({
    page,
  }) => {
  ...
  test("register with mismatched passwords shows client-side error", async ({
    page,
  }) => {
    await page.goto("/register");
    await page.waitForLoadState("networkidle");
  ...
  test("register form validation — empty fields prevent submission", async ({
    page,
  }) => {
    await page.goto("/register");
    await page.waitForLoadState("networkidle");
