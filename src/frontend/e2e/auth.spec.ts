...
test.describe("Authentication — Login & Logout", () => {
  ...
  test("login with valid admin credentials redirects to /library @smoke", async ({
    page,
  }) => {
  ...
  test("protected route /settings redirects unauthenticated user to /login", async ({
    page,
  }) => {
  ...
  test("login page redirects to /library if already authenticated", async ({
    page,
  }) => {
    // First, log in
    await loginViaUI(page, USERS.admin.email, USERS.admin.password);
